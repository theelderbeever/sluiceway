...
mod implementation {
    ...
    impl RunGuard {
        pub(crate) fn new(topology: &'static str, pipeline_id: &PipelineId) -> Self {
            metrics::gauge!("sluiceway_pipeline_active", "topology" => topology, "pipeline_id" => pipeline_id.as_str().to_owned()).increment(1.0);
            Self {
                topology,
    ...
    impl Drop for RunGuard {
        fn drop(&mut self) {
            metrics::gauge!("sluiceway_pipeline_active", "topology" => self.topology, "pipeline_id" => self.pipeline_id.as_str().to_owned()).decrement(1.0);
        }
    }
    ...
    impl Drop for StageTimer {
        fn drop(&mut self) {
            metrics::histogram!(
                "sluiceway_stage_duration_seconds",
    ...
    pub(crate) fn record(topology: &'static str, pipeline_id: &PipelineId) {
        metrics::counter!("sluiceway_records_total", "topology" => topology, "pipeline_id" => pipeline_id.as_str().to_owned()).increment(1);
    }
    ...
    pub(crate) fn batch(
        ...
        metrics::counter!(
            "sluiceway_batches_total",
            "topology" => topology,
            "pipeline_id" => pipeline_id.as_str().to_owned(),
            "reason" => reason
        )
        .increment(1);
        metrics::histogram!("sluiceway_batch_records", "topology" => topology, "pipeline_id" => pipeline_id.as_str().to_owned())
    ...
    pub(crate) fn sink_delivery(topology: &'static str, pipeline_id: &PipelineId, success: bool) {
        ...
        metrics::counter!(
            "sluiceway_sink_deliveries_total",
            "topology" => topology,
            "pipeline_id" => pipeline_id.as_str().to_owned(),
            "status" => status
        )
